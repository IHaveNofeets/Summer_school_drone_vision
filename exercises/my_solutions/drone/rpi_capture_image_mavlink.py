...
DO_VISION = False # If True, will perform vision processing on the captured images. This is currently a work in progress and not fully functional yet.
###############################################
# Standard Imports                            #
###############################################
import time
...
class DroneControl:
    ...
    def capture(self):
        ...
        if not self.debounce:
            img_loc = self.folder_loc+'raw/img_'+str(self.index)
            ...
            if DO_VISION:
                img = cv2.imread(img_loc+'.jpg')
                vision.do_vision(img, self.folder_loc, self.index)
    ...
    def run(self):
        ...
        # Kill MAVlink connection when Ctrl-C is prer'ssed (results in a lock)
